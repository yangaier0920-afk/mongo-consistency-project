...
本节结果用于展示趋势和报告结构。由于 RYW 和 WFR 脚本已经修正，正式提交前建议在同一 Docker 环境中统一重跑并替换最终数据。

### 5.1 Normal 场景

| 模型 | 弱配置违背 | 弱配置违背率 | 强配置违背 | 强配置违背率 |
|---|---:|---:|---:|---:|
| RYW | 997 / 1000 | 99.70% | 0 / 1000 | 0.00% |
| MR | 142 / 2000 | 7.10% | 0 / 2000 | 0.00% |
| MW | 0 / 1000 | 0.00% | 0 / 1000 | 0.00% |
| WFR | 0 / 1000 | 0.00% | 0 / 1000 | 0.00% |

Normal 场景下，RYW 和 MR 已经明显暴露 weak configuration 的 stale read 问题。MW 未观察到违背。WFR 在本机临时副本集重跑中未出现违背，可能因为单机复制延迟很小、观察窗口较窄。旧 CSV 中曾标记 WFR weak normal 为 `52/1000`，但该数字应使用修正后的观察顺序重新验证。
